...
def signed_distance_sphere(x, y, z, r, x_0, y_0, z_0):
    ...
    # ###############
    # TODO: Implement
    return np.sqrt(np.sum(np.square([x-x_0, y-y_0, z-z_0]), axis=0)) - r
    # ###############
...
def signed_distance_torus(x, y, z, R, r, x_0, y_0, z_0):
    ...
    # ###############
    # TODO: Implement
    x_2 = np.power(x-x_0, 2)
    y_2 = np.power(y-y_0, 2)
    z_2 = np.power(z-z_0, 2)
    alpha = np.sqrt(x_2+z_2)-R
    alpha_2 = np.power(alpha, 2)
    return np.sqrt(alpha_2+y_2)-r
...
def signed_distance_atom(x, y, z):
    ...
    # ###############
    # TODO: Implement
    s_1 = signed_distance_sphere(
        x, y, z, proton_radius, proton_center[0], proton_center[1], proton_center[2])
    s_2 = signed_distance_sphere(
        x, y, z, electron_radius, electron_center[0], electron_center[1], electron_center[2])
    t = signed_distance_torus(x, y, z, orbit_radius, orbit_thickness,
                              proton_center[0], proton_center[1], proton_center[2])
    return np.amin([s_1, s_2, t], axis=0)
    # ###############
