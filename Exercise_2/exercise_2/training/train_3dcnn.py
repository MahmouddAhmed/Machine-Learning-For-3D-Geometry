    ...
    for epoch in range(config['max_epochs']):
        for i, batch in enumerate(trainloader):
            ...
            loss_total = torch.zeros(
                [1], dtype=batch['voxel'].dtype, requires_grad=True).to(device)
            for output_idx in range(prediction.shape[1]):
                ...
                partial_prediction = prediction[:, output_idx, :]
                loss_total = loss_total + \
                    loss_criterion(partial_prediction, labels)
            ...
            if iteration % config['validate_every_n'] == (config['validate_every_n'] - 1):
                ...
                for batch_val in valloader:
                    ...
                    predicted_label = prediction.argmax(dim=2)

                    # TODO: keep track of total / correct / loss_total_val
                    total += len(labels)
                    correct += (labels == predicted_label[:, 0]).sum()
                    for output_idx in range(prediction.shape[1]):
                        partial_prediction = prediction[:, output_idx, :]
