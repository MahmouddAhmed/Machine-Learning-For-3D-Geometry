...
def train(model, trainloader, valloader, device, config):
    ...
    loss_criterion.to(device=device)

    # TODO Declare optimizer
    optimizer = Adam(lr=config['learning_rate'], params=model.parameters())
    ...
    for epoch in range(config['max_epochs']):
        for i, batch in enumerate(trainloader):
            ...
            ShapeNetParts.move_batch_to_device(batch=batch, device=device)
            ...
            predicteds = prediction.transpose(2, 1)

            loss_total = loss_criterion(predicteds, labels)
            ...
            if iteration % config['validate_every_n'] == (config['validate_every_n'] - 1):
                ...
                for batch_val in valloader:
                    ...
                    labels = batch_val['segmentation_labels']
                    inputs = batch_val['points']
                    ...
                    predicted_label = torch.argmax(predicteds, dim=1)

                    total += predicted_label.numel()
                    correct += (predicted_label ==
                                batch_val['segmentation_labels']).sum().item()

                    part_ious = []
                    for part in range(ShapeNetParts.num_classes):
                        I = torch.sum(torch.logical_and(
                            predicted_label == part, batch_val['segmentation_labels'] == part))
                    ...
                    ious.append(torch.mean(torch.stack([torch.tensor(elem, device=device) if type(
                        elem) == int else elem for elem in part_ious])))

                    loss_val += loss_criterion(prediction.transpose(2, 1),
                                               batch_val['segmentation_labels']).item()

                accuracy = 100 * correct / total
                iou = torch.mean(torch.stack(ious)).item()
                print(
                    f'[{epoch:03d}/{i:05d}] val_loss: {loss_val / len(valloader):.3f}, val_accuracy: {accuracy:.3f}%, val_iou: {iou:.3f}')
...
def main(config):
    ...
    # Create Dataloaders
    train_dataset = ShapeNetParts(
        'train' if not config['is_overfit'] else 'overfit')
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset,   # Datasets return data one sample at a time; Dataloaders use them and aggregate samples into batches
        # The size of batches is defined here
    ...
    val_dataset = ShapeNetParts(
        'val' if not config['is_overfit'] else 'overfit')
    ...
    if config['resume_ckpt'] is not None:
        model.load_state_dict(torch.load(
            config['resume_ckpt'], map_location='cpu'))
    ...
    Path(
        f'exercise_2/runs/{config["experiment_name"]}').mkdir(exist_ok=True, parents=True)
