...
class ShapeNetParts(torch.utils.data.Dataset):
    ...
    dataset_path = Path(
        "exercise_2/data/shapenetcore_partanno_segmentation_benchmark_v0/")
    # mapping for ShapeNet ids -> names
    class_name_mapping = json.loads(
        Path("exercise_2/data/shape_parts_info.json").read_text())
    classes = sorted(class_name_mapping.keys())
    part_id_to_overall_id = json.loads(Path.read_text(
        Path(__file__).parent.parent / 'data' / 'partid_to_overallid.json'))
    ...
    def __init__(self, split):
        ...
        self.items = Path(
            f"exercise_2/data/splits/shapenet_parts/{split}.txt").read_text().splitlines()

    def __getitem__(self, index):
        item = self.items[index]

        pointcloud, segmentation_labels = ShapeNetParts.get_point_cloud_with_labels(
            item)
    ...
    def get_point_cloud_with_labels(shapenet_id):
        ...
        file_path_1 = os.path.join(
            ShapeNetParts.dataset_path, category_id, "points", str(shape_id)+".pts")
        pc = np.loadtxt(file_path_1, dtype=np.float32)
        file_path_2 = os.path.join(
            ShapeNetParts.dataset_path, category_id, "points_label", str(shape_id)+".seg")
        seg = np.loadtxt(file_path_2, dtype=np.int64)
        indices = np.random.choice(a=range(pc.shape[0]), size=1024)
        pc = pc[indices]
        seg = seg[indices]
