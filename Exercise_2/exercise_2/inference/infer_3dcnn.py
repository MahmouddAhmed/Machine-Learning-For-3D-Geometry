...
class InferenceHandler3DCNN:
    ...
    def infer_single(self, voxels):
        ...
        input_tensor = torch.from_numpy(
            voxels).float().unsqueeze(0).unsqueeze(0)

        # TODO: Predict class
        prediction = self.model(input_tensor)
        prediction = prediction.max(axis=2)[1]
        prediction = prediction[0].mode()[0].item()

        # print(torch.max(prediction, dim=2)[1])
        # print(torch.max(prediction,axis=2)[1])
