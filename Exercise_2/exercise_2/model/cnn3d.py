    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride):
        ...
        self.model = nn.Sequential(
            ...
            nn.Conv3d(in_channels=out_channels,
                      out_channels=out_channels, kernel_size=(1, 1, 1)),
            nn.ReLU(),
            nn.Conv3d(in_channels=out_channels,
                      out_channels=out_channels, kernel_size=(1, 1, 1)),
            nn.ReLU()
...
class ThreeDeeCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        for d in range(backbone_features.shape[2]):
            for h in range(backbone_features.shape[3]):
                for w in range(backbone_features.shape[4]):
                    partial_predictor = self.partial_predictors[d *
                                                                self.feature_cube_side ** 2 + h * self.feature_cube_side + w]

                    # TODO: get prediction for object for backbone feature at d, h, w
                    object_features = backbone_features[:, :, d, h, w]
        ...
        full_prediction = self.full_predictor(
            backbone_features.view(batch_size, -1))
