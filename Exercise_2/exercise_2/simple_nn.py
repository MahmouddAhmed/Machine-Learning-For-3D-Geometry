...
class SimpleDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split):
        if split == "val":
            self.samples, self.labels = generate_toy_data(1024)
        elif split == "train":
            self.samples, self.labels = generate_toy_data(4096)

    def __getitem__(self, idx):
        return np.expand_dims(self.samples[idx], 0), self.labels[idx]
...
def train(model, train_dataloader, val_dataloader, device, config):
    ...
    for epoch in range(config['max_epochs']):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            input_data, target_labels = input_data.to(
                device=device), target_labels.to(device=device)
            ...
            if iteration % config['validate_every_n'] == (config['validate_every_n'] - 1):
                ...
                for batch_val in val_dataloader:
                    ...
                    loss_val += loss_criterion(prediction,
                                               target_labels).item()
...
def main(config):
    ...
    # Create Dataloaders
    # TODO Instantiate Dataset in train split
    train_dataset = SimpleDataset(split='train')
    ...
    # TODO Instantiate Dataset in val split
    val_dataset = SimpleDataset(split='val')
