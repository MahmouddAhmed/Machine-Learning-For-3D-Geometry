...
class InferenceHandlerDeepSDF:
    ...
    def get_model(self):
        ...
        model.load_state_dict(torch.load(
            self.experiment / "model_best.ckpt", map_location='cpu'))
        model.eval()
    ...
    def get_latent_codes(self):
        ...
        """
        latent_codes = torch.nn.Embedding.from_pretrained(torch.load(
            self.experiment / "latent_best.ckpt", map_location='cpu')['weight'])
    ...
    def reconstruct(self, points, sdf, num_optimization_iters):
        ...
        latent = torch.ones(1, self.latent_code_length).normal_(
        ...
        for iter_idx in range(num_optimization_iters):
            ...
            batch_indices = perm[: self.num_samples]
            ...
            latent_plus_points = torch.cat([latent_codes, batch_points], dim=1)
        ...
        vertices, faces = evaluate_model_on_grid(
            model, latent.squeeze(0), self.device, 64, None)
    ...
    def interpolate(self, shape_0_id, shape_1_id, num_interpolation_steps):
        ...
        latent_code_indices = torch.LongTensor(
            [train_items.index(shape_0_id), train_items.index(shape_1_id)]).to(self.device)
        ...
        for i in range(0, num_interpolation_steps + 1):
            ...
            w1 = i/num_interpolation_steps
            w2 = 1-w1
            interpolated_code = w2 * \
                latent_codes[0, :] + w1 * latent_codes[1, :]
            # reconstruct the shape at the interpolated latent code
            evaluate_model_on_grid(model, interpolated_code, self.device,
                                   64, self.experiment / "interpolation" / f"{i:05d}_000.obj")
    ...
    def infer_from_latent_code(self, latent_code_index):
        ...
        latent_code_indices = torch.LongTensor(
            [latent_code_index]).to(self.device)
        latent_codes = train_latent_codes(latent_code_indices)

        # reconstruct the shape at latent code
        vertices, faces = evaluate_model_on_grid(
            model, latent_codes[0], self.device, 64, None)
