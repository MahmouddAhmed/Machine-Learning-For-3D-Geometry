...
class InferenceHandler3DEPN:
    ...
    def infer_single(self, input_sdf, target_df):
        ...
        input_sdf = input_sdf.clip(-3, 3)
        target_df = target_df.clip(0, 3)
        with torch.no_grad():
            ...
            input_sdf_2 = np.repeat(
                input_sdf[np.newaxis, :, :, :, ], 2, axis=0)
            input_sdf_2[0] = np.abs(input_sdf_2[0])
            input_sdf_2[1] = np.sign(input_sdf_2[1])
            input_sdf_2 = torch.from_numpy(input_sdf_2)
            input_sdf_2 = torch.unsqueeze(input=input_sdf_2, dim=0)
            reconstructed_df = self.model(input_sdf_2)
            reconstructed_df = torch.exp(reconstructed_df)
            reconstructed_df -= 1
        ...
        reconstructed_mesh = marching_cubes(
            reconstructed_df.squeeze(0).numpy(), level=1)
