...
def train(model, train_dataloader, val_dataloader, device, config):
    ...
    loss_criterion_test = loss_criterion_test.to(device=device)

    loss_l1 = torch.nn.SmoothL1Loss()
    loss_l1 = loss_l1.to(device=device)
    ...
    for epoch in range(config['max_epochs']):
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            ShapeNet.move_batch_to_device(batch=batch, device=device)
            optimizer.zero_grad()
            input_sdf = batch['input_sdf']
            reconstruction = model(input_sdf)
            ...
            loss = loss_l1(reconstruction, target)
            loss.backward()
            optimizer.step()
            # Logging
            ...
            if iteration % config['print_every_n'] == (config['print_every_n'] - 1):
                print(
                    f'[{epoch:03d}/{batch_idx:05d}] train_loss: {train_loss_running / config["print_every_n"]:.6f}')
                train_loss_running = 0.
            ...
            if iteration % config['validate_every_n'] == (config['validate_every_n'] - 1):
                ...
                for batch_val in val_dataloader:
                    ...
                    loss_val += loss_criterion_test(reconstruction,
                                                    target).item()
                ...
                print(
                    f'[{epoch:03d}/{batch_idx:05d}] val_loss: {loss_val:.6f} | best_loss_val: {best_loss_val:.6f}')
