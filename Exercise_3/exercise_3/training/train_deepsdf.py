...
def train(model, latent_vectors, train_dataloader, device, config):
    ...
    for epoch in range(config['max_epochs']):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            num_points_per_batch = batch['points'].shape[0] * \
            ...
            batch_latent_vectors = latent_vectors(batch['indices']).unsqueeze(
                1).expand(-1, batch['points'].shape[1], -1)
            ...
            latent_plus_points = torch.cat(
                [batch_latent_vectors, points], dim=1)
            predicted_sdf = model(latent_plus_points)
            ...
            code_regularization = torch.mean(torch.norm(
                batch_latent_vectors, dim=1)) * config['lambda_code_regularization']
            if epoch > 100:
            ...
            if iteration % config['print_every_n'] == (config['print_every_n'] - 1):
                ...
                if train_loss < best_loss:
                    ...
                    torch.save(latent_vectors.state_dict(
            ...
            if iteration % config['visualize_every_n'] == (config['visualize_every_n'] - 1):
                # Set model to eval
                model.eval()
                latent_vectors_for_vis = latent_vectors(torch.LongTensor(
                    range(min(5, latent_vectors.num_embeddings))).to(device))
                for latent_idx in range(latent_vectors_for_vis.shape[0]):
                    # create mesh and save to disk
                    evaluate_model_on_grid(model, latent_vectors_for_vis[latent_idx, :], device, 64,
...
def main(config):
    ...
    if config['resume_ckpt'] is not None:
        model.load_state_dict(torch.load(
            config['resume_ckpt'] + "model_best.ckpt", map_location='cpu'))
        weights_dd = torch.load(
            config['resume_ckpt'] + "latent_best.ckpt", map_location='cpu')
        latent_vectors.load_state_dict(weights_dd)
        # model.load_state_dict(torch.load(
        #     config['resume_ckpt'] + "_model.ckpt", map_location='cpu'))
        # latent_vectors = torch.nn.Embedding.from_pretrained(torch.load(
        #     config['resume_ckpt'] + "_latent.ckpt", map_location='cpu'))
    ...
    Path(
        f'exercise_3/runs/{config["experiment_name"]}').mkdir(exist_ok=True, parents=True)
